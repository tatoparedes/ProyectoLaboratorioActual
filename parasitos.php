...
<head>
    ...
    <title>Catálogo de Parásitos | Cepario Virtual - IESTP Trujillo</title>
...
<body>
            ...
            <section class="management-content">
                <div class="catalog-header-flex">
                    <div class="catalog-title-box">
                        <h2>Catálogo de Parásitos 🪱</h2>
                        <p>Administra la clasificación taxonómica y características de los especímenes de parásitos registrados.</p>
                    </div>
                    <button type="button" class="btn-add-primary" onclick="abrirModalParasito()">
                        <i class="fas fa-plus"></i> Añadir nuevo Parásito
                    </button>
                </div>

                <!-- Grid de Especímenes (Cards) -->
                <div class="specimens-grid" id="grid-parasitos">
                    <div style="grid-column: 1/-1; text-align:center; padding: 40px; color:#64748b;">
                        <i class="fas fa-spinner fa-spin fa-2x"></i><br><br>Cargando catálogo de parásitos...
                    </div>
                </div>
            </section>
        </div>
    ...
    <!-- Modal Flotante: Añadir / Editar Parásito -->
    <div class="modal-custom-overlay" id="modal-parasito">
        <div class="modal-custom-card">
            <div class="modal-custom-header">
                <h3 id="modal-titulo"><i class="fas fa-microscope"></i> Añadir Nuevo Parásito 🔬</h3>
                <button type="button" class="modal-close-btn" onclick="cerrarModalParasito()">&times;</button>
            </div>
            <form id="form-parasito" enctype="multipart/form-data">
                ...
                <div class="modal-custom-body">
                    <div class="form-grid-2col">
                        ...
                        <div class="form-group-custom form-group-full">
                            <label for="mFoto">Fotografía Microscópica:</label>
                        ...
                        <div class="form-group-custom">
                            ...
                            <input type="text" id="mNombreComun" name="mNombreComun" placeholder="Ej. Nigua o Pique / Ascaris">
                        </div>
                        <div class="form-group-custom">
                            <label for="mNombreCientifico">Nombre Científico:</label>
                            <input type="text" id="mNombreCientifico" name="mNombreCientifico" placeholder="Ej. Tunga penetrans / Ascaris lumbricoides">
                        </div>
                        ...
                        <div class="form-group-custom">
                            ...
                            <input type="text" id="new_mMuestra" name="new_mMuestra" class="input-new-text" placeholder="Ej. Heces humanas / Piel...">
                        </div>
                        ...
                        <div class="form-group-custom form-group-full">
                            ...
                            <input type="text" id="new_mColoracion" name="new_mColoracion" class="input-new-text" placeholder="Ej. Lugol, Técnica Graham...">
                        </div>
                ...
                <div class="modal-custom-footer">
                    <button type="button" class="btn-cancel" onclick="cerrarModalParasito()">Cancelar</button>
                    <button type="submit" class="btn-save" id="btn-submit-parasito">Guardar Parásito</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById("form-parasito").addEventListener("submit", function(e) {
                e.preventDefault();
                guardarParasito();
            });
        });
        ...
        function abrirModalParasito(datos = null) {
            const modal = document.getElementById("modal-parasito");
            const form = document.getElementById("form-parasito");
            form.reset();
            ...
            if (datos) {
                document.getElementById("modal-titulo").innerHTML = '<i class="fas fa-edit"></i> Editar Parásito 🔬';
                ...
                document.getElementById("modal-titulo").innerHTML = '<i class="fas fa-microscope"></i> Añadir Nuevo Parásito 🔬';
        ...
        function cerrarModalParasito() {
            document.getElementById("modal-parasito").classList.remove("active");
        }

        function cargarParasitos() {
            const grid = document.getElementById("grid-parasitos");
            fetch("controladores/listar_parasitos.php")
                .then(res => res.json())
                .then(res => {
                    if (res.status === 'ok') {
                        grid.innerHTML = '';
                        if (res.data.length === 0) {
                            grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; padding:50px; color:#64748b;">No hay especímenes de parásitos registrados. Haz clic en "Añadir nuevo Parásito" para agregar uno.</div>';
                            return;
                        }
                        ...
                        res.data.forEach(item => {
                            ...
                            const imgHtml = fotoSrc 
                                ? `<img src="${fotoSrc}" alt="${item.mNombreComun || 'Parásito'}">`
                            ...
                            card.innerHTML = `
                                ...
                                <div class="specimen-body">
                                    ...
                                    <div class="specimen-actions-bar">
                                        <button type="button" class="btn-action btn-edit-table" onclick='abrirModalParasito(${JSON.stringify(item)})'>✏️ Editar</button>
                                        <button type="button" class="btn-action btn-delete-table" onclick="eliminarParasito(${item.mCodigo})">🗑️ Eliminar</button>
                                    </div>
                                </div>
                            `;
                ...
                .catch(err => {
                    grid.innerHTML = `<div style="grid-column:1/-1; color:#ef4444; text-align:center; padding:30px;">Error de red al cargar parásitos.</div>`;
                });
        }

        function guardarParasito() {
            const form = document.getElementById("form-parasito");
            const formData = new FormData(form);

            fetch("controladores/guardar_parasito.php", {
                method: "POST",
                body: formData
            })
            .then(res => res.json())
            ...
            .catch(err => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error de Red',
                    text: 'No se pudo guardar el parásito.'
                });
            });
        }

        function eliminarParasito(id) {
            Swal.fire({
                title: '¿Eliminar Parásito?',
                ...
                if (result.isConfirmed) {
                    fetch("controladores/eliminar_parasito.php", {
                        method: "POST",
